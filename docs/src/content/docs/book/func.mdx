...
You can express everything except `lisp-style-lists` in Tact that would be compatible with FunC's `get`-methods.
...
Conversion of arguments for `get` methods is straightforward. Each argument is mapped _as-is_ to a FunC argument, and each tuple is mapped to a [Struct](/book/structs-and-messages#structs).
